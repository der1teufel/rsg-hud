...
CreateThread(function()
    while true do
        Wait(500)
        if LocalPlayer.state.isLoggedIn and incinematic == false and inBathing == false and inClothing == false and IsAppActive(`MAP`) ~= 1 and showUI then
            local show = true
...
CreateThread(function()
    while true do
        ...
        if IsPedOnMount(PlayerPedId()) or IsPedOnVehicle(PlayerPedId()) or IsPedSittingInAnyVehicle(PlayerPedId()) then
...
RegisterNetEvent('hud:client:ShowAccounts', function(type, amount)
    ...
    elseif type == 'bloodmoney' then
        SendNUIMessage({
            action = 'show',
            type = 'bloodmoney',
            bloodmoney = format_int(string.format("%.2f", amount))
        })
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        ...
        bloodmoneyAmount = format_int(string.format("%.2f",PlayerData.money['bloodmoney']))
